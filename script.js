document.addEventListener("DOMContentLoaded", function () {
  ...
  async function checkManifest() {
    ...
    try {
      const response = await fetch(
        `https://api.github.com/repos/SteamAutoCracks/ManifestHub/branches/${gameId}`
      );
      ...
      if (response.status === 200) {
        ...
        const workerUrl =
          "https://manifesthub-bridge.sadabsiperkhan.workers.dev/";
        const gameName = appNames[parseInt(gameId)] || "Unknown Game";

        // Pointing to your Worker for counting and logging
        const trackingUrl = `${workerUrl}?download=${gameId}&name=${encodeURIComponent(
          gameName
        )}`;
        ...
        if (appDepots[appIdNum] && appDepots[appIdNum].length > 0) {
          await typeText(
            `> 🔑 Found ${appDepots[appIdNum].length} Lua keys in local database!\n`
          );
  ...
  requestForm.addEventListener("submit", async function (e) {
    ...
    try {
      ...
      if (response.ok && result.status === "success") {
        formFeedback.textContent =
          "✅ Request sent! Thank you. (Cooldown 60s)";
